# ORDA Refactoring
- 깃허브 저장소 주소: https://github.com/orda-refactoring/orda
- 조원: 장하늬, 최지원
- 활용된 기술: Django, Python, AWS(EC2, RDS[PostgreSQL, PostGIS], Elasticache[Redis]), gunicorn, NGINX, JS, HTML/CSS

## 개선점
### 백엔드
<br>

![Untitled](ORDA_readme_img/orda_arc.jpg)

<br>

1. 기능/코드 개선
  - 회원가입 기능 개선(reCAPTCHA, 아이디 중복 여부 검사 기능 등 도입)
  - 필터링 기능 수정(POST method 대신 GET method 적용)
  - 프로필 페이지 조회시 갱신되는 Level 기능 문제점 보완(점수 획득시, 유저 레벨 조정 함수 호출하게 수정)
  - 함수 생성 및 분리
    - geoJSON 데이터 직렬화의 함수화 및 기존 VIEW들에서 분리
    - 외부 API 호출 함수를 기존 상세 View에서 분리시켜 비동기화 처리
2. 기능 추가
  - 산과 유저의 현 위치 간 거리 계산 기능 구현
  - 유저 맞춤형 메인 페이지 VIEW 구현
3. 데이터 캐싱 및 쿼리 최적화
4. 배포 환경 수정
...
1. 코드 변경 / 개선
 -  모든 페이지 css코드를 SCSS로 변경
 -  코드 재사용 150줄 가량 제거
2. UI 변경 및 페이지 추가
 - 추천에 특화된 main 페이지 추가
 - 모든 페이지 iphone SE사이즈까지 반응형 디자인
 - post_index, post_create, header 등 페이지의 UI 정리
